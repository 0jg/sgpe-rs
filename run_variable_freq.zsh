...
# Center frequency and anisotropy for the trap
CENTER_FREQ=25
...
while IFS=, read -r mu t; do
	...
	echo "Running simulation with mu=$mu nK, T=$t nK, freq_x=${CENTER_FREQ}Hz, freq_y=${CENTER_FREQ}Hz (± anisotropy)"
	$PROGRAM_PATH $t $mu false $CENTER_FREQ $CENTER_FREQ &
	active_jobs+=($!)
done < "$MU_T_FILE"
